document.addEventListener('DOMContentLoaded', () => {
  ...
  // Fallback: reset if inventory is invalid
  ...
  function saveToXML(items) {
    ...
    items.forEach(i => {
      xml += '<item>';
      xml += `<name>${i.name}</name><cost>${i.cost}</cost><profit>${i.profit}</profit><unit>${i.unit}</unit><co2>${i.co2}</co2>`;
      inventory.forEach(inv => {
        xml += `<${inv.name}>${i[inv.name] || 0}</${inv.name}>`;
      });
      xml += '</item>';
    });
    xml += '</products>';
  ...
  function loadFromXML(file) {
    ...
    reader.onload = () => {
      ...
      const items = [...doc.querySelectorAll('item')].map(node => {
        ...
        inventory.forEach(inv => {
          const el = node.querySelector(inv.name);
          product[inv.name] = el ? parseFloat(el.textContent) : 0;
        });
        return product;
      });
      tableBody.innerHTML = '';
